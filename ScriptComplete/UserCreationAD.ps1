...
if ($TestingPathwayCSV)
{
    foreach ($user in $UpdatedUsersList)
    {
        ...
        try 
        {
            $NewUserCreated = New-ADUser -Name $user.Name -Surname $user.Surname -DisplayName $user.DisplayName -Country $user.Country -EmployeeID $user.EmployeeID -EmployeeNumber $user.EmployeeNumber -EmailAddress $user.EmailAddress -Department $user.Department -AccountPassword $SecureStringConvert -ErrorAction Stop
            $PathOfOU = "OU=" + $NewUserCreated.Department + "," + $DcInformation.DistinguishedName
            Move-ADObject -Identity $NewUserCreated.ObjectGUID -TargetPath $PathOfOU
        }
        catch 
        {
            Write-Host "Failed to either create or move the user '$($user.Name)'. Error: $($_.Exception.Message)"
        }    
    }
}
}
...
Write-Host '''
1. Check the Creation of the Organizational Groups in your Department File were successful.
2. Ensure that the users populate into those OUs.
3. Confirm each user has the correct property details. 
'''
